-- AutoBank Full Database Setup (run manually with psql)
-- Step 1: create the database (connect as postgres superuser)
-- psql -U postgres -c "CREATE DATABASE autobank;"
-- Step 2: apply schema
-- psql -U postgres -d autobank -f db/schema.sql
...
CREATE TABLE IF NOT EXISTS accounts (
    ...
    status VARCHAR(20) DEFAULT 'ACTIVE' NOT NULL,  -- ACTIVE | INACTIVE | FROZEN
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
...
CREATE TABLE IF NOT EXISTS transactions (
    ...
    type VARCHAR(30) NOT NULL,  -- DEPOSIT | WITHDRAWAL | ADJUSTMENT | CORRECTION | TRANSFER_IN | TRANSFER_OUT
    amount NUMERIC(15,2) NOT NULL,
    balance_before NUMERIC(15,2) NOT NULL,
    balance_after NUMERIC(15,2) NOT NULL,
    description TEXT,
    status VARCHAR(20) DEFAULT 'ACTIVE' NOT NULL,  -- ACTIVE | REVERSED | CORRECTED | MODIFIED
    operator_id INT REFERENCES users(id),
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    reference_id INT REFERENCES transactions(id)  -- for corrections/reversals
);
...
CREATE TABLE IF NOT EXISTS daily_sessions (
    ...
    status VARCHAR(20) DEFAULT 'OPEN' NOT NULL,  -- OPEN | CLOSED | VERIFIED
...
CREATE TABLE IF NOT EXISTS loans (
    ...
    due_date DATE,
    status VARCHAR(20) DEFAULT 'ACTIVE' NOT NULL,  -- ACTIVE | CLOSED | OVERDUE | WRITTEN_OFF
    operator_id INT REFERENCES users(id)
);
...
CREATE TABLE IF NOT EXISTS distribution_records (
    ...
    amount NUMERIC(15,2),
    status VARCHAR(20) DEFAULT 'PENDING' NOT NULL,  -- PENDING | CREDITED | FAILED | DUPLICATE
    error_message TEXT,
    transaction_id INT REFERENCES transactions(id)
);
